  ...
  class API < Base::API
    resources :flows do
      ...
      get do
        ...
        flows = Flow.where(safe_params.permit(:initial)).order(sort => order)

        if safe_params[:query].present?
          flows = flows.search(safe_params[:query])
        end

        { flows: Flow::Entity.represent(paginate(flows), only: return_fields, display_type: safe_params[:display_type]) }
      end
