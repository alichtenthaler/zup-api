...
describe Cases::API, versioning: true do
  ...
  describe 'to get all cases' do
    ...
    let(:other_flow) do
      flow = create(:flow, :with_resolution_state, title: 'Other', initial: true, steps: [build(:step_type_form_without_fields), build(:step_type_form)])
      create(:field, step: flow.steps.first, title: 'company_age', field_type: 'integer')
      flow.publish(user)
      flow.reload
    end
    let(:flow) do
      flow = create(:flow, :with_resolution_state, initial: true, steps: [build(:step_type_form_without_fields), build(:step_type_form)])
    ...
    context 'with authentication' do
      context 'with query search' do
        ...
        before { get '/cases', { query: 'Other' }, auth(user) }
      ...
      context 'with cases completed' do
        let(:kase1_resolution_state) { kase1.initial_flow.resolution_states.first }
        let(:kase2_resolution_state) { kase2.initial_flow.resolution_states.first }

        before do
          add_permision_to_user(user, kase1.case_steps.first.step.id)
          put "/cases/#{kase1.id}/finish", { resolution_state_id: kase1_resolution_state.id }, auth(user)
          put "/cases/#{kase2.id}/finish", { resolution_state_id: kase2_resolution_state.id }, auth(user)
        end

        context 'with filter by completed' do
          let(:case_ids) { parsed_body['cases'].map { |k| k['id'] } }

          before { get '/cases', { completed: true }, auth(user) }
          it { expect(response.status).to be_a_success_request }
          it { expect(parsed_body['cases'].count).to eql 2 }
          it { expect(case_ids).to include kase1.id }
          it { expect(case_ids).to include kase2.id }
        ...
        context 'with filter by resolution_state_id' do
          let(:case_ids) { parsed_body['cases'].map { |k| k['id'] } }

          before { get '/cases', { resolution_state_id: kase1_resolution_state.id, completed: true }, auth(user) }
          it     { expect(response.status).to be_a_success_request }
          it     { expect(parsed_body['cases'].count).to eql 1 }
          it     { expect(case_ids).to include kase1.id }
          it     { expect(case_ids).to_not include kase2.id }
